		...
		<view class="box">
			<view class="name">
				<view class="name1">
					...
					<view class="inp">
						<picker @change="bmType" :value="bmTypeIndex" :range="bmArr" range-key="label"
							v-if="bmArr.length>0">
			...
			<view class="name">
				<view class="name1">
					...
					<view class="inp">
						<picker @change="zdType" :value="index" range-key="dictLabel" :range="zhanArr"
							v-if="zhanArr.length>0">
			...
			<view class="name">
				<view class="name1">
					...
					<view class="inp">
						<picker @change="sbType" :value="sbTypeIndex" :range="sbArr" range-key="dictLabel"
							v-if="sbArr.length>0">
			...
			<view class="name">
				<view class="name1">
					...
					<view class="inp">
						<picker @change="trzd" range-key="dictLabel" :value="trzdIndex" :range="trzdArr"
							v-if="trzdArr.length>0">
			...
			<view class="name">
				<view class="name1">
					...
					<view class="inp">
						<input type="text" v-model="params.zwidth" value="" placeholder="请输入宽度" />
					</view>
				</view>
			</view>
			<view class="name">
			...
			<view class="name">
				<view class="name1">
					...
					<view class="inp">
						<picker @change="listArea" :value="listIndex" :range="list" v-if="list.length>0"
							range-key="name">
			...
			<view class="name" v-if="listIndex!=-1">
				<view class="name1">
					<view class="text">
						乡镇
					</view>
			...
			<view class="name" v-if="cunlist.length>0">
				<view class="name1">
					...
					<view class="inp">
						<picker @change="cun" :value="cunIndex" :range="cunlist" range-key="name"
							v-if="cunlist.length>0">
...
<script>
	export default {
		...
		methods: {
			getZhanDianHttp() {
				...
				this.$http.get(url).then(res => {
					...
					if (res.code == 200 && res.data.ws) {
						this.params = {
							deptId: null, //所属区域id  必填
							wsname: res.data.ws.wsname, //站点名称 必填
							stationtype: null, //站点类型 必填
							address: null, //地址
							lon: res.data.ws.lon, //经度  必填
							lat: res.data.ws.lat, //纬度  必填
							zheight: res.data.ws.zheight, // 长度
							zwidth: res.data.ws.zwidth, // 宽度
							area: null, // 面积
							district: null, // 所在区
							town: null, // 所在镇
							street: null, // 所在村
							...
							townCodet: null, // 所在镇编码
							streetCode: null, // 所在村编码
							park: res.data.ws.park, // 园区名称
							name: res.data.ws.name, // 基地联系人
							phone: res.data.ws.phone, // 联系方式
							areaNum: res.data.ws.areaNum, // 温室号
							waterNum: null, // 表号
							type: null, // 设施类型
							soilType: null, // 土壤质地
							irrigateMode: null, // 灌溉方式
						...
						this.getDetaultBm(res.data.ws.district)
						setTimeout(()=>{
							this.getDetaultZd(res.data.ws.stationtype)
						},100)
						setTimeout(()=>{
							this.getDetaultSs(res.data.ws.type)
						},200)
						setTimeout(()=>{
								this.getDetaultTr(res.data.ws.soilType)
						},300)
					setTimeout(()=>{
							this.getDetaultQu(res.data.ws.district)
					},400)
						
						setTimeout(() => {
							this.getDetaultZhen(res.data.ws.town)
						}, 500)
						setTimeout(() => {
							this.getDetaultCun(res.data.ws.street)
						}, 1300)
			...
			// 查找默认部门
			getDetaultBm(district) {
				var num = this.bmArr.findIndex((value, index, arr) => {
					return district.includes(value.label)
				})
				this.bmTypeIndex = num
				this.params.deptId = this.bmArr[this.bmTypeIndex].id
			},
			// 查找默认站点类型
			getDetaultZd(stationtype) {
				var num = this.zhanArr.findIndex((value, index, arr) => {
					return stationtype == value.dictValue
				})
				this.index = num
			...
			// 查找默认设施类型
			getDetaultSs(type) {
				var num = this.sbArr.findIndex((value, index, arr) => {
					return type == value.dictValue
				})
				this.sbTypeIndex = num

				this.params.type = this.sbArr[this.sbTypeIndex].dictValue
			},
			// 查找默认土壤类型
			getDetaultTr(type) {
				var num = this.trzdArr.findIndex((value, index, arr) => {
					return type == value.dictValue
			...
			// 查找默认区
			getDetaultQu(district) {
				var num = this.list.findIndex((value, index, arr) => {
					return district == value.name
				})
				this.listIndex = num
				this.params.district = this.list[this.listIndex].name
				this.params.districtCode = this.list[this.listIndex].id
			},
			// 查找默认镇
			getDetaultZhen(district) {
				this.getZhenHttp(this.list[this.listIndex].id)
				setTimeout(() => {
					var num = this.xzlist.findIndex((value, index, arr) => {
						return district == value.name
					})
					this.xzIndex = num
			...
			// 查找默认村
			getDetaultCun(district) {
				this.getCunHttp(this.xzlist[this.xzIndex].id)
				setTimeout(() => {
					var num = this.cunlist.findIndex((value, index, arr) => {
						return district == value.name
					})
					this.cunIndex = num
			...
			submit() {
				if (this.params.deptId && this.params.wsname && this.params.stationtype && this.params.lon && this.params
					.lat) {

					this.params.id = this.onLoadParams
					// console.log(this.params, "保存参数")
					this.$http.put('/area/amendArea', this.params).then(res => {
						// console.log(res,"保存接口")
						if (res.code == 200) {
							uni.showToast({
								title: '保存成功,即将转回上一级页面',
								icon: 'success'
							})
							setTimeout(() => {
								uni.navigateTo({
			...
			getZhenHttp(pid) {
				this.$http.get('/regionalization/getAreaInfo', {
					pid: pid
				}).then(res => {
					// console.log(res, "所属镇")
					if (res.code == 200 && res.data.length > 0) {
